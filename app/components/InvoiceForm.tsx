...
type InvoiceForm = {
  existingInvoiceData?: InvoicesWithCustomersType;
  setExistingInvoiceData?: React.Dispatch<
    React.SetStateAction<InvoicesWithCustomersType | null>
  >;
...
const InvoiceForm = ({
  ...
  useEffect(() => {
    ...
    const { customer, labour, discount, invoiced_products } =
      existingInvoiceData;
    ...
    if (setExistingInvoiceData) setExistingInvoiceData(null);
  }, [existingInvoiceData]);
    ...
    return (
      <>
        {[...Array(apvCount)].map((e, i) => {
          const rowItem = allProductValues[i];
